...
from src.Hungman.exceptions.invalid_operation_error import InvalidOperationError
from src.Hungman.game_status import GameStatus


class Game:

    def __init__(self, allowed_misses: int = 6):
        if 5 > allowed_misses > 8:
            raise ValueError("Number of allowed_misses should be between 5 and 8")
    ...
    def guess_letter(self, letter: str) -> Iterable[str]:
        ...
        if self.game_status != GameStatus.IN_PROGRES:
